...
def setAxParameters(ax, option):
	...
	'''
	ax.set_xticks(np.linspace(option['x_lim'][0],
	                          option['x_lim'][1],
	                          11
	                         )
	             )
	ax.set_yticks(np.linspace(option['y_lim'][0],
	                          option['y_lim'][1],
	                          11
	                         )
	             )
	...
	ax.xaxis.set_tick_params(labelsize=tick_label_size, pad=-5)
	ax.xaxis.set_major_formatter(FormatStrFormatter('%.2f'))
	ax.set_yticklabels(ax.get_yticks(), rotation = 45)
	ax.yaxis.set_tick_params(labelsize=tick_label_size, pad=0)
