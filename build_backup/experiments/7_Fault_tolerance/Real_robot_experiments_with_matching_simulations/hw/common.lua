...
package.path = package.path .. ";/home/harry/code-mns2.0/SoNS2.0-SR/build/experiments/7_Fault_tolerance/Real_robot_experiments_with_matching_simulations/?.lua"
--]]
if robot.params.hardware ~= "true" then
	package.path = package.path .. ";/home/harry/code-mns2.0/SoNS2.0-SR/build/experiments/7_Fault_tolerance/Real_robot_experiments_with_matching_simulations/simu/?.lua"
end
...
return function()
	...
	if sons.parentR == nil then
		...
		elseif state == 4 then
			for id, ob in ipairs(sons.avoider.obstacles) do
				if ob.type == target_type then
					sons.setGoal(sons, ob.positionV3 - vector3(0.5, 0.7, 0), ob.orientationQ)
					--[[
		...
		if left > middle and right < middle and 
		   left < 9 and right > -9 then
			if sons.stabilizer.referencing_robot == nil then
				local newGoalPositionV3 = sons.goal.positionV3
				local Y_offset = 0.7
...
return function()
	if sons.api.stepCount == 500 and robot.random.uniform() < 0.3 then
		sons.reset(sons)
		fail_return = false
	end
