...
package.path = package.path .. ";/home/harry/code-mns2.0/SoNS2.0-SR/build/testing/exp_09_scalability_light/?.lua"
...
function showParentInThick(sons, color)
...
function showChildrenInThick(sons, color, withoutBrain)
...
function step()
	...
	if robot.id == "pipuck76" and sons.api.stepCount == 100 then 
		--api.debug.showParent(sons, up_color)
		showParentInThick(sons, up_color)
	end 

	for _, msgM in ipairs(sons.Msg.getAM("ALLMSG", "led_signal_up")) do
		if sons.parentR ~= nil then 
			--api.debug.showParent(sons, up_color)
			showParentInThick(sons, up_color)
		else
			--api.debug.showChildren(sons, down_color, true) -- without brain true
			showChildrenInThick(sons, down_color, true)
		end
	end

	if sons.parentR ~= nil then for _, msgM in ipairs(sons.Msg.getAM(sons.parentR.idS, "led_signal")) do
		--api.debug.showChildren(sons, down_color, true) -- without brain true
		showChildrenInThick(sons, down_color, true)
	end end

	-- debug
	api.debug.showChildren(sons, nil, true) -- color nil, without brain true
			...
			if sons.robotTypeS == "pipuck" and sons.stabilizer.referencing_me ~= true then 
				...
				for i, ob in ipairs(sons.avoider.obstacles) do
					if ob.type == gate_brick_type and ob.positionV3:length() < 0.3 then
						sons.goal.transV3 = sons.goal.transV3 + vector3(1,0,0):rotate(ob.orientationQ) * 0.5
					end
...
function create_led_node(sons)
	...
	local down_color = signal_color
	return function()
		if sons.api.stepCount == 100 then
			if robot.id == "pipuck76" then
				sons.Msg.send(sons.parentR.idS, "led_signal_up")
				sons.api.debug.drawRing(up_color, vector3(0,0,0), 0.15, true)
				sons.api.debug.drawRing(up_color, vector3(0,0,0), 0.175, true)
				sons.api.debug.drawRing(up_color, vector3(0,0,0), 0.20, true)
			end
		end
		...
		if sons.parentR ~= nil then for _, msgM in ipairs(sons.Msg.getAM(sons.parentR.idS, "led_signal")) do
			for idS, childR in pairs(sons.childrenRT) do
				sons.Msg.send(idS, "led_signal")
				sons.api.debug.drawRing(down_color, vector3(0,0,0), 0.15, true)
				sons.api.debug.drawRing(down_color, vector3(0,0,0), 0.175, true)
				sons.api.debug.drawRing(down_color, vector3(0,0,0), 0.20, true)
			end
		end end
		-- signal led
		--[[
